...
class PlanoContratacaoController extends BaseController
{
    ...
    {
        $data = $request->all();
        $data['contratacao']['user_id'] = $this->getUserId();
    ...
    public function show($id){
        try{
            $plano = $this->planoContratacaoRepository->find($id);
            if($plano['data']['user_id'] != $this->getUserId()){
                throw new \Exception('Contratacão inexistente!');
            }
            return $this->planoContratacaoRepository->find($id);
        }catch (ModelNotFoundException $e){
